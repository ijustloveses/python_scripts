    ...
    def run(self):
        ...
        while True:
            try:
                ...
                # SystemCall is handled by Scheduler
                if isinstance(result, SystemCall):
                    return result
                # In case of generator type, means we met "trampoline"
                # save current coroutine on stack and call the new coroutine
                ...
                if not self.stack:
                    raise
                # Pop the last coroutine and continue,
                # instead of killing the whole task
                self.sendval = None
                self.target = self.stack.pop()
...
class Scheduler(object):
    ...
    def __init__(self):
        ...
        self.read_waiting = {}   # for non-blocking io
        self.write_waiting = {}  # for non-blocking io
    ...
    def mainloop(self):
        ...
        """
        self.new(self.iotask())    # Launth I/O polls, runs with every other tasks
        while self.taskmap:
            task = self.ready.get()
            try:
                result = task.run()
